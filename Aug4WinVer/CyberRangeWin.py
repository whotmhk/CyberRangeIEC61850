...
def run_script_in_terminal(script_name):
    ...
    if platform.system() == "Windows":
        if script_name.endswith('.py'):
            terminal_command = f"start cmd.exe /k python {script_path}"
        else:
            terminal_command = f"start cmd.exe /k {script_path}"
    else:  # Linux (Ubuntu)
        if script_name.endswith('.py'):
            terminal_command = f"gnome-terminal -- python3 {script_path}"
        else:
            terminal_command = f"gnome-terminal -- {script_path}"
...
def update_compliance_results(ot_vars, sc_vars, fc_vars, results_frame):
    ...
    # Create left frame for text results
    ...
    # Create pie chart in the right frame
...
root = tk.Tk()
root.title("IEC 61850 Hands-on Practice & Tutorials")
...
setup_tab(goose_tab, [
    ...
    ("goose_device_vlans.py", "Identifying VLAN ID, Prority and Multicast address"),
    ...
    ("IEDExplorer.exe", "Induce High stNum and sqNum sequence")
])
