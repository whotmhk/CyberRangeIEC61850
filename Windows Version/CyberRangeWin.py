...
def run_script_in_terminal(script_name):
    ...
    if platform.system() == "Windows":
        terminal_command = f"start cmd.exe /k run_as_admin.bat {script_path}"
    else:  # Linux (Ubuntu)
        if script_name.endswith('.py'):
            terminal_command = f"gnome-terminal -- bash -c 'python3 {script_path}; exec bash'"
        else:
...
setup_tab(mms_tab, [
    ("IEDExplorer.exe", "Reconnaissance and Explore the IDEs information"),
...
setup_tab(goose_tab, [
    ...
    ("Publisher.exe", "Induce High stNum and sqNum sequence")
])
